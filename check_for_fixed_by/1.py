...
if __name__ == "__main__":
    jsonFile = open('../entire_corpus/entire_corpus_1.json')
    jsonData = json.load(jsonFile)
    updatedJsonData = json.load(open('../entire_corpus_python_patches/1.json'))
    for i, item in enumerate(jsonData):
        if(len(updatedJsonData)<3737142):
            ...
            break
    with open('../entire_corpus_python_patches/1.json', 'w') as w:
