...
if __name__ == "__main__":
    jsonFile = open('../entire_corpus/entire_corpus_6.json')
    jsonData = json.load(jsonFile)
    updatedJsonData = json.load(open('../entire_corpus_python_patches/6.json'))
    for i, item in enumerate(jsonData):
        if(len(updatedJsonData)<373742):
            ...
            break
    with open('../entire_corpus_python_patches/6.json', 'w') as w:
